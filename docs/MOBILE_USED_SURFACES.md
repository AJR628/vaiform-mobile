...
- Last verified against: both repos on 2026-03-09

Generated from source on 2026-03-09.
...
| `POST /api/story/finalize` | Render confirmation in the storyboard header | Body stays exactly `{ sessionId }`. Header includes `X-Idempotency-Key`, generated when a real finalize attempt begins, stored in a stable ref for the active attempt, and reused for the built-in 503 retry and recovery path. | Reads `ok`, `retryAfter`, `code`, `status`, `message`, and `shortId`. On `TIMEOUT`, `NETWORK_ERROR`, or `IDEMPOTENT_IN_PROGRESS`, the screen now switches to backend-backed recovery via `GET /api/story/:sessionId`, keeps the same active attempt key, and only clears it after a same-attempt terminal `renderRecovery` state. On success it refreshes credits and cross-navigates to `LibraryTab -> ShortDetail` with `{ shortId }`. | `client/screens/StoryEditorScreen.tsx:915-1099`, `client/api/client.ts:696-804` |
