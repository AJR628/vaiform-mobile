...
| `POST /api/story/finalize` | Render confirmation in the storyboard header using the current session `billingEstimate.estimatedSec` | Body stays exactly `{ sessionId }`. Header includes `X-Idempotency-Key`, generated when a real finalize attempt begins, stored in a stable ref for the active attempt, and reused for the built-in 503 retry and recovery path. | Reads `ok`, `retryAfter`, `code`, `status`, `message`, and `shortId`. On success it also reads additive `data.billing.billedSec` when present for success messaging, refreshes usage, and cross-navigates to `LibraryTab -> ShortDetail` with `{ shortId }`. On `TIMEOUT`, `NETWORK_ERROR`, or `IDEMPOTENT_IN_PROGRESS`, the screen now switches to backend-backed recovery via `GET /api/story/:sessionId`, keeps the same active attempt key, and only clears it after a same-attempt terminal `renderRecovery` state. | `client/screens/StoryEditorScreen.tsx`, `client/api/client.ts` |
