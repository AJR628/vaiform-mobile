...
| `POST /api/story/estimate` | Immediately before showing the final render confirmation copy | `{ sessionId }` | Unwraps `data`, updates local session state, and reads refreshed `billingEstimate.estimatedSec` only. Mobile does not read or depend on backend-internal `billingEstimateProbe`. Error path reads `ok`, `success`, `message`. | `client/screens/StoryEditorScreen.tsx`, `client/api/client.ts` |
| `POST /api/story/finalize` | Render confirmation in the storyboard header, after a successful render-intent estimate refresh | Body stays exactly `{ sessionId }`. Header includes `X-Idempotency-Key`, generated when a real finalize attempt begins, stored in a stable ref for the active attempt, and reused for the built-in 503 retry and recovery path. | Reads `ok`, `retryAfter`, `code`, `status`, `message`, and `shortId`. On success it also reads additive `data.billing.billedSec` when present for success messaging, refreshes usage, and cross-navigates to `LibraryTab -> ShortDetail` with `{ shortId }`. On `TIMEOUT`, `NETWORK_ERROR`, or `IDEMPOTENT_IN_PROGRESS`, the screen now switches to backend-backed recovery via `GET /api/story/:sessionId`, keeps the same active attempt key, and only clears it after a same-attempt terminal `renderRecovery` state. | `client/screens/StoryEditorScreen.tsx`, `client/api/client.ts` |
