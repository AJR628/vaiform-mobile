...
# Beat Editor Keyboard â€” Final Verify + Implementation Plan
...
- **No double-save:** Submit â†’ blur must not cause two API calls.
...
# Step 1 â€” Verify Enter submission (with logs)
...
**A) On the beat-edit TextInput** (lines 513â€“537):
...
2. Tap a beat â†’ focus TextInput â†’ type â†’ press **Done**.
3. Check logs:
...
**Record:** iOS submit fires? ___ | Android submit fires? ___ â†’ Strategy: **A2** / **Fallback**

---

# Step 2 â€” Implement B1 (floating editor above keyboard)
...
- **Delete** the entire block  
  ...
  from inside `View (previewSection)` (currently lines 506â€“545). The preview section should only contain `View (previewContainer)` with the thumbnail.
...
# Step 3 â€” Implement Enter = Save

**File:** `client/screens/StoryEditorScreen.tsx` (beat-edit `TextInput` inside the overlay)

## Option A2 (single-line submit) â€” use if Step 1 shows submit fires on both platforms
...
## Fallback (multiline + Enter as submit) â€” use if Android submit is flaky
...
- In **`onChangeText`**:
  - If `text` contains `\n`:
    ...
    - Call `handleSaveBeat(selectedBeat.sentenceIndex, "submit", cleaned)` (or extend `handleSaveBeat` to accept an optional `draftOverride` and use it instead of `beatTexts[sentenceIndex]` when provided â€” since `setState` is async, the save must use the cleaned value explicitly).
    - `textInputRef.current?.blur();` and `Keyboard.dismiss();`
  - Else: update `beatTexts` as today.
- **Keep** `onBlur` â†’ `handleSaveBeat(selectedBeat.sentenceIndex, "blur")`.
- Add `ref={textInputRef}`.

---

# Step 4 â€” Prevent double-save + avoid unnecessary API calls
...
- **Draft:**  
  ...
  if `!draft` â†’ `showError("Beat text cannot be empty"); return;`
...
**Output:** After Step 1, record â€œStrategy: A2â€ or â€œStrategy: Fallbackâ€ and implement the matching Option in Step 3.

---

# Manual test checklist

1. **Clean run** â†’ Story Editor â†’ tap a beat. Keyboard opens.
2. **Input above keyboard:** Beat TextInput remains **visible above** the keyboard (thumbnails may be covered). **Pass / Fail**
3. **Enter = Save:** Type â†’ press **Done** â†’ **one** save, keyboard dismisses, **no** newline. **Pass / Fail**
4. **No-changes Done:** Focus â†’ **donâ€™t** type â†’ **Done** â†’ keyboard dismisses, **no** API call. **Pass / Fail**
5. **Tap-away:** Type â†’ tap timeline â†’ save runs, keyboard dismisses. **Pass / Fail**
